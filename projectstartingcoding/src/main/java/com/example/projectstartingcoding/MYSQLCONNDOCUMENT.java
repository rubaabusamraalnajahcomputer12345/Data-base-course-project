...
public class MYSQLCONNDOCUMENT {
    ...
    @FXML
    public static ObservableList<DOCUMENT> getDatausers() throws SQLException {
        ...
        try {
            PreparedStatement PS = conn.prepareStatement("SELECT * FROM lawpro.Document ");
            ResultSet rs = PS.executeQuery();
            //Alert A=new Alert(Alert.AlertType.CONFIRMATION,"CONNECT SUCCESSFULL", ButtonType.OK);
            //  A.show();

            while (rs.next()) {
                list.add(new DOCUMENT(Integer.parseInt(rs.getString("Document_id")), rs.getString("Document_title"), rs.getString("Creation_date"), (Integer.parseInt(rs.getString("Case_id")))));
                // Alert A=new Alert(Alert.AlertType.CONFIRMATION,"CONNECT SUCCESSFULL", ButtonType.OK);
                // A.show();
